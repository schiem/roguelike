...
/*
Explanation of how the screen works:
There is a chunk map, which holds all of the chunks.  We need a way
to go from chunk->screen.  However, since we always want the character
to be centered, there will be times when the screen consists of half
of one chunk and half of another.
...
So, there is a buffer, which essentially holds all of the surrounding chunks.  A buffer might look like:

123
456
789

Where each number is a different chunk.  But, we only want part of this to get written to the screen.
So, we have a canvas, which is the size of the screen taken from the buffer with the character at the center.
...
void Canvas::refresh()
{
    //If the character has gone out of bounds of the chunk,t hen the chunk and buffer need updated
    if(out_of_bounds()) {
        update_chunk();
        update_buffer();
    }
    //This section checks if the character is in a dungeon, or the overworld
    //In the dungeon, the character moves around the screen.
    //In the overworld, a calculation is done to keep the character at the center of the screen.
...
*/
/*
Here, we update the buffer based on the chunks surrounding the character's current chunk.  This should be broken into several functions.
This will be called whenever the character moves into a new chunk, so that the buffer reflects the chunks surrounding the characters current one.
*/
...
void Canvas::update_buffer()
{
    ...
    //loops through the characters y chunk coordinate, +/- 1.  If the character was in chunk 7, this would loops through 6, 7, 8. 
        ...
        for(int row=main_char.get_chunk_x()-1;row<=main_char.get_chunk_x()+1;row++) {
            ...
            for (int a=0;a<STARTING_HEIGHT;a++) {
                for (int b=0;b<STARTING_WIDTH;b++) {
                    /*
                        This line is a bit confusing.  What I need is to write the contents of the chunkt to the appropriate place in the buffer.
                        A and B represent the Y and X of individual tiles.  So, for each chunk, the X and Y are written to the buffer.
                        The chunks that we're iterating through are essentially a 3x3 array.  Each chunk needs to start being written at the
                        appropriate location (e.g. the second chunk needs to start where the first one left off...), which is where the x and y
                        variables come in.  Starting at 0, the x and y are multiplied by the width/height of the chunk, which is added to 
                        a and b to get where the tile from the chunk corresponding to a and b should be written to the buffer.  For example:
